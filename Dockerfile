...
COPY packages/adapters/letta-code-local/package.json packages/adapters/letta-code-local/
...
RUN npm install --global --omit=dev @anthropic-ai/claude-code@latest @openai/codex@latest @letta-ai/letta-code@latest opencode-ai \
  && mkdir -p /paperclip \
